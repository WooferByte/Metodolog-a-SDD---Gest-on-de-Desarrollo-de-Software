...
All unhandled exceptions SHALL be caught and returned as standardized RFC 7807 JSON error responses.

#### Scenario: HTTP exception (e.g., 404)
- **WHEN** client requests nonexistent endpoint
- **THEN** response status is 404 and body contains `{"type": "...", "title": "Not Found", "detail": "...", "status": 404}`

#### Scenario: Validation error (e.g., bad JSON)
- **WHEN** client sends invalid JSON or request body
- **THEN** response status is 422 and body includes field-level validation errors in RFC 7807 format

#### Scenario: Internal server error
- **WHEN** server encounters unhandled exception
- **THEN** response status is 500 and body contains generic error message (internals not exposed)
